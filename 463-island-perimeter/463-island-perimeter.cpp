class Solution {
    bool inBounds(int a, int b, int &m, int &n){ //Checking if the indices a and b are valid
...
public:
    int islandPerimeter(vector<vector<int>>& grid) {
        ...
        for(int i=0; i<m; i++){
            for(int j=0; j<n; j++){
                if(grid[i][j]==1){ //Finding the starting position(or the first '1' that is encountered)
                    start={i, j};
                    break;
                }
            }
        ...
        while(!Q.empty()){ 
            ...
            if(inBounds(x-1, y, m, n) && grid[x-1][y]!=0){
                if(grid[x-1][y]==1) Q.push({x-1, y}); //If position is not visited and it is a '1'
                grid[x-1][y]=-1;
                sides--;
            }
